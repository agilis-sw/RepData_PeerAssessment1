...
```
The <b>mean</b> is <b>`r meanvalue`</b>, and the <b>median</b> is <b>`r medianvalue`</b>.
...
The interval corresponding to the highest average number of steps (<b>`r maxavgsteps`</b>) is <b>`r maxinterval`</b>, corresponding to <b>`r maxintervalstr` hours</b>.  
...
```{r getmissingrowcount,echo=TRUE}
missingrowcount = nrow(df) - nrow(dfc)
missingrowpct = (missingrowcount/nrow(df))*100
```
  
Per above, there are <b>`r missingrowcount`</b> observations having missing values, 
corresponding to `r missingrowpct` percent.
...
```{r imputenumberofsteps,echo=TRUE}
for (i in 1:nrow(df)) {
        if (is.na(df$steps[i])) {
                hrmin = df$interval[i]
...
```
The new <b>mean</b> is <b>`r meanvalue2`</b>, and the new <b>median</b> is <b>`r medianvalue2`</b>.  
...
given that incomplete cases represented only `r missingrowpct` percent of the observations.  The new distribution also appears to be more uniform and the mean and median pretty
...
Code:  
```{r generateweekdayweekend,echo=TRUE}
